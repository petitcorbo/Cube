...
button1 = tk.Button(top, text="Revolution", command=lambda: revolution(int(s1.get())))
button1.grid(row=0, column=1)
spinbox = tk.Spinbox(top, from_=1, to=10)
spinbox.grid(row=0, column=0)

button2 = tk.Button(top, text="Rotation", command=rotation)
button2.grid(row=1, column=1, rowspan=2)
scale1 = tk.Scale(top, label="δ", variable=sigma, from_=0, to=360, resolution=15, orient='horizontal')
scale1.grid(row=1, column=0)
scale2 = tk.Scale(top, label="θ", variable=theta, from_=0, to=360, resolution=15, orient='horizontal')
