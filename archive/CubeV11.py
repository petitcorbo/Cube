...
def create_cube(side=250):
    ...
    for sub, color in zip(scheme[0], color_faces):
        canvas.create_polygon([points[i][:2] for i in sub], outline='black', fill=color, tags=color)
...
button_rotation = tk.Button(text="Console", command=console,
    bg=color_buttons, fg=color_fg, activebackground=color_canvas, activeforeground=color_fg
    )
